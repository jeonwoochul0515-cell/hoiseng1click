...
exports.diagnoseCodefAuth = diagnoseCodefAuth;
...
async function diagnoseCodefAuth() {
...
async function collectWithCodef(token, accountList) {
    ...
    const successOrgs = successList.map((s) => s.organization).filter(Boolean);
    const skippedOrgsFull = errorList.map((e) => ({
        organization: e.organization,
        code: e.code,
        message: e.message,
    }));
    // 실제 데이터 있는 기관 집계 (debts/assets의 meta에서 추출)
    const orgsWithData = new Set();
    for (const d of debts) {
        const orgName = d.creditor ?? d.source;
        if (orgName)
            orgsWithData.add(String(orgName));
    }
    for (const a of assets) {
        const orgName = a.meta?.bankName ?? a.meta?.insurerName ?? a.name;
        if (orgName)
            orgsWithData.add(String(orgName));
    }
    // 인증 성공했지만 데이터 없는 기관 = 전체 성공 - 데이터 있는 기관
    const emptyDataOrgs = successOrgs.filter((o) => !Array.from(orgsWithData).some((d) => d.includes(o) || o.includes(d)));
    return {
        connectedId: cid,
        debts,
        assets,

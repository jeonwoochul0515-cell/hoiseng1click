...
// ──────────────────────────────────────────────
// 8. 중지명령 신청서 (채무자회생법 제593조 제1항 제2호)
// ──────────────────────────────────────────────

interface SuspensionOrderData {
  court: string;
  clientName: string;
  clientSSN: string;
  clientAddr: string;
...
export async function createSuspensionOrderDoc(data: SuspensionOrderData): Promise<Buffer> {
  const children: (Paragraph | Table)[] = [
    createTitle("중지명령 신청서"),
    p("(채무자회생법 제593조 제1항 제2호)", { alignment: AlignmentType.CENTER, before: 0, after: 300 }),
    emptyLine(),
    sectionHeading("1. 신청인(채무자) 인적사항"),
    kvTable([
      ["성명", data.clientName],
  ...
  children.push(
    emptyLine(),
    sectionHeading("5. 신청 이유"),
    p(data.reason || "채무자는 개인회생절차 개시 신청을 하였으나, 진행 중인 강제집행 등으로 인해 최저생계비 유지가 불가능한 상황이므로, 채무자회생법 제593조 제1항 제2호에 따라 강제집행 등의 중지를 구합니다."),
    emptyLine(),
    sectionHeading("6. 소명자료"),
  );
...
interface ExemptionDecisionData {
  court: string;
  clientName: string;
  clientSSN: string;
  clientAddr: string;
...
export async function createExemptionDecisionDoc(data: ExemptionDecisionData): Promise<Buffer> {
  const children: (Paragraph | Table)[] = [
    createTitle("면제재산결정 신청서"),
    p("(채무자회생법 제580조 제3항, 민사집행법 제195조)", { alignment: AlignmentType.CENTER, before: 0, after: 300 }),
    emptyLine(),
    sectionHeading("1. 신청인(채무자) 인적사항"),
    kvTable([
      ["성명", data.clientName],
  ...
  if (data.exemptionCount > 0) {
    data.exemptionItems.forEach((item) => {
      children.push(
        p(`${item.no}. ${item.name} (${item.type}) — ${item.value}`, { bold: true }),
        bullet(`   사유: ${item.reason}`),
      );
    });
  } else {
...
export async function buildDocx(
  ...
  switch (docType) {
    ...
    case "suspension_order":
      return createSuspensionOrderDoc(data as unknown as SuspensionOrderData);
    case "exemption_decision":
      return createExemptionDecisionDoc(data as unknown as ExemptionDecisionData);
    default:

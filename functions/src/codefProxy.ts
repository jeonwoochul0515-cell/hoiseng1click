...
function buildAccountList(
  ...
  const seen = new Set<string>(); // 보험 어그리게이터 중복 방지
...
export async function callCodef(token: string, endpoint: string, body: object): Promise<unknown> {
  async function doFetch(authToken: string): Promise<{ status: number; decoded: string }> {
    ...
    try {
      ...
      if (!res.ok) console.log(`[CODEF] Full error: ${decoded}`);
...
async function collectWithCodef(
  ...
  const acctOrgs = Array.isArray(accountList)
    ? accountList.map((a: any) => `${a.organization}(${a.businessType})`).join(", ")
    : "unknown";
...
// ---------------------------------------------------------------------------
export async function handleCodefTestConnection(_req: Request, res: Response) {
  ...
  try {
    ...
    results.api = { ok: true, elapsed: Date.now() - start, code: (resp as any)?.result?.code };
  } catch (err: any) {

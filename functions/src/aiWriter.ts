...
export async function handleAiGenerate(req: Request, res: Response): Promise<void> {
  try {
    const { field, keywords, context } = req.body as {
      field: AiField;
      keywords: string;
      context?: {
        name?: string;
    ...
    if (context) {
      const parts: string[] = [];
      if (context.name) parts.push(`신청인: ${context.name}`);
      if (context.totalDebt) parts.push(`총 채무액: ${context.totalDebt.toLocaleString()}원`);
      if (context.debtCount) parts.push(`채권자 수: ${context.debtCount}건`);
      if (context.job) parts.push(`직업: ${context.job}`);
      if (context.income) parts.push(`월 소득: ${context.income.toLocaleString()}원`);
      if (context.family) parts.push(`가족 수: ${context.family}명`);
      if (parts.length) {
        userPrompt += `\n\n참고 정보:\n${parts.join("\n")}`;
      }
    }
    ...
    if (!apiKey) {
      const dummyTexts: Record<AiField, (kw: string) => string> = {
        debtHistory: (kw) =>
          `신청인은 ${kw.split(/[,، ]+/)[0] || "생활비 부족"}(으)로 인하여 채무가 발생하게 되었습니다. 처음에는 소액의 신용대출로 시작하였으나, 이후 수입이 감소하면서 기존 채무의 이자 납부조차 어려워졌고, 부득이하게 추가 대출을 받아 기존 채무를 상환하는 악순환에 빠지게 되었습니다. 현재 총 채무가 감당할 수 없는 수준에 이르러 정상적인 방법으로는 채무 변제가 불가능한 상황이 되었습니다. 신청인은 깊이 반성하고 있으며, 개인회생 절차를 통하여 성실히 변제할 것을 다짐합니다.`,
        propertyChanges: (kw) =>
          `신청인은 ${kw || "채무 상환을 위해"} 부득이하게 보유 재산을 처분하였습니다. 처분 대금은 전액 채무 상환 및 생활비에 사용하였으며, 현재 특별히 은닉하거나 빼돌린 재산은 없습니다.`,
        repayWillingness: (kw) =>
          `신청인은 ${kw.split(/[,، ]+/)[0] || "성실한 변제"}을(를) 통해 채무를 갚아나갈 것을 굳게 다짐합니다. 향후 불필요한 지출을 최대한 줄이고, 가능한 범위에서 추가 수입 확보를 위해 노력하겠습니다. 법원이 인가해 주시는 변제계획에 따라 매월 변제금을 성실히 납부하여, 채권자분들께 조금이라도 보답하고자 합니다. 다시는 과도한 채무를 부담하지 않도록 검소한 생활을 유지하겠습니다.`,
        jobChange: (kw) =>

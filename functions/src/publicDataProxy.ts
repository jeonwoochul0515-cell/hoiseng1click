...
function extractLawdCd(address: string): string | null {
  ...
  for (const [key, code] of Object.entries(LAWD_CD)) {
    ...
    if (address.includes(shortKey) && shortKey.length >= 2) return code;
  }
  return null;
}
...
// 부동산 공시가격 조회
// ──────────────────────────────────────────────
export async function handlePropertyLookup(req: Request, res: Response) {
  const address = (req.query.address as string) ?? "";
  const area = Number(req.query.area ?? 84);
  const propertyType = (req.query.type as string) ?? "apt";
...
async function fetchByPnu(
  apiKey: string, pnu: string, propertyType: string, address: string, area: number,
) {
  let apiUrl: string;
  ...
  for (const dealYmd of months) {
    ...
    try {
      ...
      if (aptName && matched.length > 1) {
        ...
        if (aptMatched.length > 0) matched = aptMatched;
      }
...
/** 주소에서 아파트 이름 추출 (선택) */
function extractAptName(address: string): string | null {
...
function fallbackSimulation(res: Response, address: string, area: number) {
...
function mileageAdjust(km: number, age: number): number {
  ...
  if (ratio < 0.7) return 1.05;   // 주행거리 적음: +5%
  if (ratio < 1.0) return 1.00;   // 평균 이하: 기본
  if (ratio < 1.3) return 0.95;   // 평균 이상: -5%
  if (ratio < 1.6) return 0.90;   // 많음: -10%
  return 0.85;                     // 매우 많음: -15%
}

export async function handleVehicleLookup(req: Request, res: Response) {
  const plate = (req.query.plate as string) ?? "";
  const model = (req.query.model as string) ?? "소나타";

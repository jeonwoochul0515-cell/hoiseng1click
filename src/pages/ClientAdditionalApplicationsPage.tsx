// B2B 법무사용 — 의뢰인별 부가신청서 3종 생성 페이지
// /clients/:clientId/additional-applications
//
...
export default function ClientAdditionalApplicationsPage() {
  ...
  useEffect(() => {
    ...
    getClient(office.id, clientId)
      .then((c) => setClient(c))
      .catch((err) => toast.error(err?.message ?? '의뢰인 로드 실패'))
      .finally(() => setLoading(false));
  }, [office?.id, clientId]);

  const hasCaseNumber = useMemo(() => {
    const parsed = parseCaseNumber(client?.caseNumber ?? '');
    return parsed.valid;
  }, [client?.caseNumber]);

  const rehabCourt = useMemo(() => {
    if (!client?.court) return '';
    return getRehabilitationCourt(client.court, client.filingDate);
  }, [client?.court, client?.filingDate]);

  const courtChanged = rehabCourt && rehabCourt !== client?.court;
  ...
  const onCaseConfirm = async (data: {
    ...
    if (!office?.id || !clientId) return;
    try {
      await updateClient(office.id, clientId, {
        court: data.court,
        caseNumber: data.fullCaseNumber,
        filingDate: data.filingDate,
      } as any);
      const updated = await getClient(office.id, clientId);
      setClient(updated);
      setShowModal(false);
  ...
  const generateDoc = async (kind: AppKind) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${label}_${client?.name ?? 'unknown'}.docx`;
  ...
  if (!client) {
    return (
      <div className="text-center py-16">
        <p className="text-sm text-gray-500">의뢰인을 찾을 수 없습니다</p>
        <button
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-16">
      <header>
        ...
        >
          <ArrowLeft size={14} /> {client.name} 의뢰인 정보로
        </button>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <FileText className="text-indigo-600" size={22} />
          부가신청서 생성
          <span className="text-base font-medium text-gray-500">— {client.name}</span>
        </h1>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="text-gray-900">{client.court ?? '미설정'}</div>
                {courtChanged && (
              ...
              <div>
                ...
                <div className="text-gray-900 font-mono">
                  {client.caseNumber || <span className="text-gray-400">미입력</span>}
                </div>
              </div>
              <div>
                <div className="text-[11px] text-gray-500 mb-0.5">접수일</div>
                <div className="text-gray-900">{client.filingDate || '미입력'}</div>
              </div>
              <div>
                <div className="text-[11px] text-gray-500 mb-0.5">의뢰인</div>
        ...
        <div className="rounded-xl border-2 border-amber-300 bg-amber-50 p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-amber-900">
            ...
            <p className="text-xs mt-1 leading-relaxed">
              ...
              전자소송 포털 → 나의전자소송에서 사건번호를 확인 후 "사건번호 등록" 버튼을 눌러주세요.
            </p>
          </div>
        </div>
      )}
            ...
            >
              <div className="flex items-start gap-4 flex-wrap">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-bold rounded-lg shrink-0 text-white ${
                    !hasCaseNumber || isGenerating
                      ? 'bg-gray-300 cursor-not-allowed'
                      : `${app.buttonColor} hover:opacity-90`
                  }`}
                >
                  {isGenerating ? (
                    <>
      ...
      <CaseNumberModal
        ...
        docLabel="부가신청서"
        initialCourt={client.court}
        initialFilingDate={client.filingDate}
